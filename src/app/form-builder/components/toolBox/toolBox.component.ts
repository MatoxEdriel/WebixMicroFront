...
export class ToolBoxComponent implements OnInit, AfterViewInit {
    ...
    const toolbox = webix.ui({
      ...
      on: {
        onBeforeDrag: (context: any, e: any) => {
          // Creamos un componente con view garantizada para el servicio
          const componentToDrag: IComponent & { view: IViewConfig } = {
            id: context.start.id,
            name: context.start.name,
            enable: context.start.enable ?? true,
            view: context.start.view ?? { view: 'text' } // fallback en caso de que no tenga view
          };

          // Enviamos al DragService
          this._dragService.startDrag(componentToDrag);
  ...
  getHtmlContent(obj: { view: IViewConfig }): string {
    ...
    switch (obj.view.view) {
      case 'text':
        return `<input type="text" placeholder="${obj.view.placeholder || ''
          }" disabled style="
